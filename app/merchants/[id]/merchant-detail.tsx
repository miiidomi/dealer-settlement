...
function PaymentStatus({
  ...
  if (status === "미입금")
    return <Badge className="bg-red-50 text-red-700">미입금</Badge>;
  if (status === "입금완료")
    return (
      <Tooltip>
        <TooltipTrigger asChild>
          <span
            tabIndex={0}
            aria-label={`입금완료 · 입금일자 ${date || "미등록"}`}
            className="inline-flex cursor-help rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300"
          >
...
export default function MerchantDetail({ merchantId }: { merchantId: number }) {
  ...
  const [rangeMode, setRangeMode] = useState(
    ["all", "month", "year", "custom"].includes(initialMode)
      ? initialMode
      : "month",
  );
  ...
  const allRange = monthRangeFrom([
    ...
    ...(data?.payments ?? [])
      .filter((row) => row.merchantId === merchantId)
      .flatMap((row) => [row.paymentDate, row.billingMonth]),
  ]);
  const range =
    rangeMode === "all"
