...
export default function SettlementApp() {
  ...
  const [rangeMode, setRangeMode] = useState(() =>
    ["all", "month", "year", "custom"].includes(initialParam("rangeMode"))
  ...
  const allRange = monthRangeFrom([
    ...(data?.merchants ?? [])
      .filter((merchant) => dealerId === null || merchant.dealerId === dealerId)
      .map((merchant) => merchant.installDate),
    ...(data?.payments ?? [])
      .filter(
        (payment) =>
          dealerId === null ||
          data?.merchants.find((merchant) => merchant.id === payment.merchantId)
            ?.dealerId === dealerId,
      )
      .flatMap((payment) => [payment.paymentDate, payment.billingMonth]),
    ...(data?.installations ?? [])
      .filter(
        (installation) =>
          dealerId === null ||
          data?.merchants.find(
            (merchant) => merchant.id === installation.merchantId,
          )?.dealerId === dealerId,
      )
      .flatMap((installation) => [
        installation.contractInstallAt,
    ...
    ...(data?.vanSettlements ?? [])
      .filter(
        (settlement) => dealerId === null || settlement.dealerId === dealerId,
      )
      .map((settlement) => settlement.settlementMonth),
  ]);
  const range =
    rangeMode === "all"
      ? { ...allRange, label: "전체 기간" }
      : rangeMode === "month"
